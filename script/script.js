...
const checkDogs = (dogsJulia, dogsKate) => {
    ...
    dogsJulia = Julia.concat(julia2);
    console.log(dogsJulia);
    let Kate = [4, 1, 15, 8, 3];
    let kate2 = [10, 5, 6, 1, 4];
    dogsKate = Kate.concat(kate2);
    console.log(dogsKate);

    let dogsJuliaUpdated = ([...dogsJulia.slice(1, -2)]);
    console.log(dogsJuliaUpdated)

    let updatedDogs = dogsKate.concat(dogsJuliaUpdated);
    console.log(updatedDogs);

    updatedDogs.forEach(function (dog, i) {
        if (dog > 3) {
...
const calcAgeHumam = function (mow) {
    // const humanAges = low.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
    // console.log(humanAges);
    const humanAge = mow.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
    console.log(humanAge)
};


calcAgeHumam([16, 6, 10, 5, 6, 1, 4])
...
const calcAverageHumanAge = function(ages) {
    ...
    const averageAge = adultdogs.reduce((arr, age) => arr + age) / adultdogs.length
